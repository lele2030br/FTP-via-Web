...
// Alterar permissões (agora funciona para arquivos e pastas)
if (isset($_POST['set_chmod'])) {
    $chmod_file = $_POST['chmod_file'];
    $chmod_mode = octdec($_POST['chmod_mode']);
    $result = ftp_set_permissions($conn_id, $dir . $chmod_file, $chmod_mode);
    if ($result) {
        $message = "Permissões alteradas!";
    } else {
        $message = "Falha ao alterar permissões.";
    }
}
...
<body>
                ...
                <tbody>
                    ...
                    <?php foreach ($items as $item): ?>
                        <tr>
                            ...
                            <td><?= $item['is_dir'] ? '-' : number_format($item['size']) . ' bytes' ?></td>
                            <td>
                                <?= htmlspecialchars($item['perms']) ?>
                                <button class="btn btn-sm btn-outline-primary" data-bs-toggle="modal" data-bs-target="#chmodModal" data-file="<?=htmlspecialchars($item['name'])?>" data-perms="<?=htmlspecialchars($item['perms'])?>">Permissões</button>
                            </td>
                            <td class="ftp-actions text-end">
                                <?php if ($item['is_dir']): ?>
                                    ...
                                    <a href="?delete=<?=urlencode($item['name'])?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('Excluir pasta?')">Excluir</a>
                                <?php else: ?>
                                    ...
                                    <a href="?rename=<?=urlencode($item['name'])?>" class="btn btn-sm btn-outline-secondary">Renomear</a>
                                    <a href="?move=<?=urlencode($item['name'])?>" class="btn btn-sm btn-outline-dark">Mover</a>
                                    <a href="?delete=<?=urlencode($item['name'])?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('Excluir arquivo?')">Excluir</a>
                                <?php endif; ?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                    <?php if (empty($items)): ?>
                        <tr>
                            <td colspan="5" class="text-center text-muted">Nenhum arquivo ou pasta.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
    ...
    <!-- Modal para alterar permissões (funciona para arquivos e pastas) -->
    <div class="modal fade" id="chmodModal" tabindex="-1" aria-labelledby="chmodModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <form method="post">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="chmodModalLabel">Alterar Permissões</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body">
                        <input type="hidden" name="chmod_file" id="chmod_file">
                        <div class="mb-3">
                            <label for="chmod_mode" class="form-label">Permissões (ex: 644 para arquivos, 755 para pastas)</label>
                            <input type="text" name="chmod_mode" id="chmod_mode" class="form-control" pattern="\d{3,4}" required>
                        </div>
                        <div class="text-muted">Use <b>755</b> para pastas e arquivos executáveis, <b>644</b> para arquivos comuns.</div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                        <button type="submit" name="set_chmod" class="btn btn-primary">Alterar</button>
                    </div>
                </div>
            </form>
        </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (chmodModal) {
            chmodModal.addEventListener('show.bs.modal', function (event) {
                ...
                document.getElementById('chmod_file').value = button.getAttribute('data-file');
                document.getElementById('chmod_mode').value = '';
            });
        }
    });
    </script>
